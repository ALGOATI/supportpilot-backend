...
export default function SettingsPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const { data, error } = await supabase
        ...
        .select("business, tone, reply_length, dashboard_language")
        .eq("user_id", user.id)
        .maybeSingle();
  ...
  async function connectWhatsApp() {
    ...
    try {
      ...
      if (resp.ok && json.ok) {
        setWaConnected(true);
        setWaAccessToken(""); // Clear token from UI after save
  ...
  async function save() {
    ...
    // Upsert: creates row if missing, otherwise updates it
  ...
  if (loading) {
    return (
      <DashboardShell title={t(dashboardLanguage, "settings")} subtitle={t(dashboardLanguage, "settings_subtitle")}>
  ...
  return (
    <DashboardShell title={t(dashboardLanguage, "settings")} subtitle={t(dashboardLanguage, "settings_subtitle")}>
      ...
      <div style={{ marginTop: 16, display: "grid", gap: 14 }}>
        ...
        >
          <div style={{ fontWeight: 900, marginBottom: 8, color: "#111827" }}>
            {t(dashboardLanguage, "business_info")}
          </div>
          ...
          <p style={{ marginTop: 8, color: "#374151" }}>
            The AI is only allowed to use what you put here.
          </p>
          <p style={{ marginTop: 4, color: "#6b7280" }}>
            Structured business data can be managed in Business Setup and is used first.
          </p>
        </div>
