...
import {
  ...
  // whatsappService needs updateConversationManualMode which is still
  // declared later in this file. async function declarations are hoisted,
  // so we wrap it in an arrow shim to defer the lookup to call time.
  const whatsappService = createWhatsAppService({
    supabaseAdmin,
    updateConversationManualMode: (...args) => updateConversationManualMode(...args),
  });
  const {
  ...
  // pipelineService is created further below (after all helper service
  // factories and after the inline conversation-map / conversation-record
  // helpers it depends on are reachable via lexical lookup).
  const {
  ...
  // Create the conversation pipeline service. It depends on a few helpers
  // (getOrCreateConversationMap, getConversationByIdForUser,
  // upsertConversationRecord) that are still declared later in this file
  // as `async function` declarations. Those are hoisted, so we wrap them
  // in arrow shims that look them up lazily at call time.
  const conversationPipelineService = createConversationPipelineService({
    ...
    normalizeBusinessType,
    buildIndustryTemplateGuidance,
    getOrCreateConversationMap: (...args) => getOrCreateConversationMap(...args),
    getConversationByIdForUser: (...args) => getConversationByIdForUser(...args),
    upsertConversationRecord: (...args) => upsertConversationRecord(...args),
    wixPaymentService,
  });
  const {
